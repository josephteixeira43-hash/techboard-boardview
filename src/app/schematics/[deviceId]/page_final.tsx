...
import { ArrowLeft, Zap, Bot, Cpu, Search } from "lucide-react";
...
export default function SchematicsProPage() {
  ...
  useGlobalHotkeys([
    {
      ...
      description: "Abrir Global Search",
    },
    {
      key: "Escape",
      handler: () => {
      ...
      const mapped = storageFiles
        ...
        .map(f => {
          ...
          return {
            ...
            type: type as "electrical_list" | "troubleshooting" | "schematic",
            url: data.publicUrl,
            device_id: deviceId,
          };
        });
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    if (canvas) searchOCR(query, ocrResults, currentPage, canvas.width, canvas.height, scale);
  }, [ocrResults, currentPage, scale, ocrReady, selectedFile, runOCR, searchOCR, clearHighlights, canvasRef]);

  const handlePaletteSelect = useCallback((name: string, pageIndex?: number) => {
    setPaletteOpen(false);
    handleSearch(name);
    if (pageIndex !== undefined && pageIndex !== currentPage) {
      setCurrentPage(pageIndex);
    }
  }, [handleSearch, currentPage, setCurrentPage]);
  ...
  return (
    ...
    >
      ...
      <header className="flex items-center justify-between px-4 py-2.5 border-b border-gray-800 z-40 shrink-0 bg-gray-900/95 backdrop-blur">
        ...
        {/* Centro: SearchBar + botão Global Search */}
        <div className="flex-1 max-w-xl mx-6 flex items-center gap-2">
          ...
          <motion.button
            ...
            onMouseLeave={e => {
              ...
              e.currentTarget.style.boxShadow = "0 0 12px rgba(0,212,255,0.06)";
            }}
            title="Global Search IA (Ctrl+K)"
          >
            <Search size={13} />
      ...
      <div className="flex flex-1 overflow-hidden">
        <ComponentSidebar
          ...
          onSelect={(f) => { setSelectedFile(f); setOcrReady(false); }}
