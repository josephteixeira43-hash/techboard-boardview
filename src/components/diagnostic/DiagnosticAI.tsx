...
// ✓ CATEGORY_COLORS agora vem da fonte única — src/lib/constants.ts
...
export default function DiagnosticAI({ deviceName, fileName, onHighlightComponent, onClose }: Props) {
  ...
  async function send(text?: string) {
    ...
    try {
      ...
      const highlights = [...new Set(content.match(compRegex) || [])] as string[]

      setMessages([...newMessages, { role: 'assistant', content, highlights }])
      if (highlights.length > 0) onHighlightComponent(highlights[0])
    } catch {
      setMessages([...newMessages, {
        role: 'assistant',
                    ...
                    {msg.highlights && msg.highlights.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-2 pt-2 border-t border-gray-700/50">
                        {msg.highlights.slice(0, 6).map((c) => {
                          // Detecta categoria pelo prefixo para colorir corretamente
                          const cat = c.startsWith('U') ? 'CPU'
                            : c.startsWith('C') ? 'OTHER'
                            : c.startsWith('R') ? 'OTHER'
                            : c.startsWith('L') ? 'OTHER'
                            : 'OTHER'
