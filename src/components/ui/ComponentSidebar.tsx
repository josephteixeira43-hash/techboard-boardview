...
export default function ComponentSidebar({
  ...
  return (
    <div className="w-64 bg-gray-900 border-r border-gray-800 flex flex-col shrink-0">
      ...
      {/* Componente selecionado pela IA */}
      <AnimatePresence>
        {selectedComponent && (
          <motion.div
          ...
          >
            <div className="p-3">
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={onClearComponent} className="text-gray-600 hover:text-white">
                  <X size={12} />
                </button>
              </div>
              <div
                ...
                style={{
                  background: `${CATEGORY_COLORS[selectedComponent.category] || CATEGORY_COLORS.DEFAULT}15`,
                  borderColor: `${CATEGORY_COLORS[selectedComponent.category] || CATEGORY_COLORS.DEFAULT}40`,
                }}
              >
                <div className="flex items-center gap-2 mb-2">
                  <div
                    ...
                    style={{
                      background: `${CATEGORY_COLORS[selectedComponent.category] || CATEGORY_COLORS.DEFAULT}25`,
                      color: CATEGORY_COLORS[selectedComponent.category] || CATEGORY_COLORS.DEFAULT,
                    }}
                  >
      ...
      <div className="px-3 py-3 border-b border-gray-800">
        ...
        >
          {uploading ? <><Loader2 size={13} className="animate-spin" />Enviando... {progress}%</> : <><Upload size={13} />Adicionar PDF</>}
        </label>
        {uploading && (
      ...
      <div className="flex-1 overflow-y-auto py-2">
        {loading ? (
          ...
          <div className="text-center py-12 px-4">
            ...
            <p className="text-xs text-gray-600">Nenhum PDF. Faça upload dos esquemas do A12.</p>
          </div>
        ) : (
                  ...
                  <motion.div
                    ...
                    className={`group flex items-center gap-2 mx-2 px-3 py-2 rounded-xl cursor-pointer transition-all mb-0.5 ${
                      selectedFile?.id === file.id
                        ? "bg-yellow-500/12 border border-yellow-500/30"
                        : "hover:bg-gray-800/60"
                    }`}
                  >
                    {selectedFile?.id === file.id && (
                      <div className="w-1 h-4 rounded-full bg-yellow-400 shrink-0" />
                    )}
                    <span className={`text-xs flex-1 truncate font-mono ${
                      selectedFile?.id === file.id ? "text-yellow-300" : "text-gray-500"
                    }`}>
                      {file.name}
                    </span>
                    <button
